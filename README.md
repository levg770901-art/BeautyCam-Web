...
4. 「切換後鏡頭」可在前／後鏡頭之間切換。
5. 前鏡頭預設鏡像，點「鏡像」可自行開關；後鏡頭預設不鏡像。
6. 偵測到多人時，各張臉會分別套用美肌處理。
7. 調整「美肌／磨皮／美白」即可即時套用。
8. 點「拍照」後，以最多 1600px 的處理尺寸輸出照片；儲存結果不會因前鏡頭鏡像預覽而左右反轉。
9. 「上傳照片」會嘗試進行臉部辨識，再以最多 1600px 處理。
10. 點「儲存照片」輸出 JPEG。
...
- Front-camera mirror preview：前鏡頭預設鏡像，且可手動開關。
- Non-mirrored capture output：鏡像僅為預覽顯示，拍照與儲存保持正常左右方向。
